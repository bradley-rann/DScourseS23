...
#3-Fold Cross Validation
tree_cv <- vfold_cv(income_train, v = 3)
...
tree_ans %<>% left_join(logit_test %>% slice(1),by=c(".metric",".estimator")) %>%
...
#3-Fold Cross Validation
nnet_cv <- vfold_cv(income_train, v = 3)
...
nnet_ans %<>% left_join(logit_test %>% slice(1),by=c(".metric",".estimator")) %>%
  mutate(alg = "neural") %>% select(-starts_with(".config"))
...
#3-Fold Cross Validation
knn_cv <- vfold_cv(income_train, v = 3)
...
knn_ans %<>% left_join(logit_test %>% slice(1),by=c(".metric",".estimator")) %>%
  mutate(alg = "neighbor") %>% select(-starts_with(".config"))
...
#3-Fold Cross Validation
svm_cv <- vfold_cv(income_train, v = 2)
...
final_svm_lasso <- finalize_workflow(svm_wf,
                                      best_acc_svm
)
...
svm_ans %<>% left_join(logit_test %>% slice(1),by=c(".metric",".estimator")) %>%
...
#Easy latex table
kable(all_ans, caption = "PS10", format = "latex", booktabs = TRUE)
